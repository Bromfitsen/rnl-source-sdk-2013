...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CRnLAxisSquadSelectMenu::CRnLAxisSquadSelectMenu(IViewPort *pViewPort) : Frame( NULL, PANEL_SQUAD_AXIS )
{
	...
	LoadControlSettings("resource/ui/rnl_axissquadselect.res");
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
CRnLAxisSquadSelectMenu::~CRnLAxisSquadSelectMenu()
{
	gameeventmanager->RemoveListener(this);
}

void CRnLAxisSquadSelectMenu::Reset()
{
...
//-----------------------------------------------------------------------------
void CRnLAxisSquadSelectMenu::ApplySchemeSettings( IScheme *pScheme )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRnLAxisSquadSelectMenu::ShowPanel(bool bShow)
{
	...
	if ( bShow )
	{
		...
		// cjd @removed - menu plays from the loadout room panel now
		/*
...
void CRnLAxisSquadSelectMenu::FireGameEvent( IGameEvent *event )
{
	if( IsVisible() )
		Update();
}

bool CRnLAxisSquadSelectMenu::NeedsUpdate( void )
{
...
//-----------------------------------------------------------------------------
void CRnLAxisSquadSelectMenu::Update( void )
{
	// our key values for the items
	KeyValues *itemData = new KeyValues("selectiondata");
	C_RnLGameTeam* pAxisTeam = (C_RnLGameTeam*)GetGlobalRnLTeam( TEAM_AXIS );
	if( !pAxisTeam )
		return;
	...
	for( int iSquad = 0; iSquad < pAxisTeam->GetNumberOfSquads(); iSquad++ )
	{
		itemCount = 0;
		pSquad = pAxisTeam->GetSquad(iSquad);
		iClassData = 0;
		pPlayer = NULL;
		...
		if( pSquad )
		{
			...
			for( int i = 0; i < pSquad->GetTotalAvailableKits(); i++ )
			{
				...
				if( iClassData > -1 )
				{
					for( int j = 0; j < pSquad->GetKitMaxCount( i ); j++ )
					{
						...
						classData->SetString( "class", pAxisTeam->GetKitDescription( iClassData ).title );
			...
			SquadGUIGetIdealProportions( iSquad, pAxisTeam->GetNumberOfSquads(), x, y, w, h );
...
void CRnLAxisSquadSelectMenu::OnSquadListItemSelected( KeyValues *data )
{
	char squadJoin[32];
	C_RnLGameTeam* pAxisTeam = (C_RnLGameTeam*)GetGlobalRnLTeam( TEAM_AXIS );
	if( !pAxisTeam )
	...
	{
		itemCount = 0;
		pSquad = dynamic_cast<C_RnLBaseSquad*>(pAxisTeam->GetSquad(iSquad));
		if( bSearch )
		{
...
void CRnLAxisSquadSelectMenu::OnCommand( const char *command )
{
...
void CRnLAxisSquadSelectMenu::SetData( KeyValues *data )
{
...
//-----------------------------------------------------------------------------
void CRnLAxisSquadSelectMenu::PaintBackground( void )
{
}