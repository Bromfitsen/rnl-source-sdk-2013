...
#include "fx.h"
	...
	//FIXME: Should be a nade spoon not a cartridge
	#define SPOON_MODEL "models/shells/spoon.mdl"
	void FX_ThrowNadeSpoon( int iSpoonType, Vector vOrigin, QAngle vAngles )
	{
		CGib *pSpoon = CREATE_ENTITY( CGib, "gib" );
		pSpoon->Spawn( SPOON_MODEL );
		pSpoon->SetBloodColor( DONT_BLEED );

		pSpoon->SetAbsOrigin( vOrigin );
		pSpoon->SetAbsAngles( vAngles );

		// 30 seconds life-time till fade out
		pSpoon->m_lifeTime = 10.0;
		// needs this to fade out
		pSpoon->SetRenderColorA( 255 );

		pSpoon->m_bForceRemove = true;
		pSpoon->SetNextThink( gpGlobals->curtime + pSpoon->m_lifeTime );
		pSpoon->SetThink ( &CGib::DieThink );

		pSpoon->SetCollisionGroup( COLLISION_GROUP_DEBRIS );
		IPhysicsObject *pPhysicsObject = pSpoon->VPhysicsInitNormal( SOLID_VPHYSICS, pSpoon->GetSolidFlags(), false );
		...
		if ( pPhysicsObject )
		{
			...
			Vector vecVelocity;
			vecVelocity[0] = vAngles[0];
			vecVelocity[1] = vAngles[1];
			vecVelocity[2] = vAngles[2];

			vecVelocity *= 75;
			//vecVelocity += GetAbsVelocity();

			AngularImpulse angImpulse = RandomAngularImpulse( 30, -30 ); 

			pSpoon->SetAbsVelocity( vecVelocity );
			pPhysicsObject->AddVelocity(&vecVelocity, NULL );
			pPhysicsObject->ApplyTorqueCenter( angImpulse );
		}
	}
	...
	)
{
...
#ifdef CLIENT_DLL
	C_RnLPlayer *pPlayer = ToRnLPlayer( ClientEntityList().GetBaseEntity( iPlayerIndex ) );
#else
	CRnLPlayer *pPlayer = ToRnLPlayer( UTIL_PlayerByIndex( iPlayerIndex ) );
#endif
	...
	if ( bDoEffects )
	{
		...
		// do the third person weapon flash here
		// TODO: different flashes for different guns
#ifdef CLIENT_DLL
		if( GetLocalPlayerIndex() != iPlayerIndex && pPlayer )
		{
			C_WeaponRnLBase *pWeapon = pPlayer->GetActiveRnLWeapon();

			if( pWeapon )
			{
				ClientEntityHandle_t hEntity = ClientEntityList().EntIndexToHandle( pWeapon->entindex() );
				FX_MuzzleEffectAttached( 0.6f, hEntity, pWeapon->LookupAttachment("muzzle_flash"), NULL, false);
			}
		}
#endif
	}
	...
	for ( int iBullet=0; iBullet < pWeaponInfo->m_iBullets; iBullet++ )
	{
		...
		*/
		x = RandomFloat( -0.25, 0.25 ) + RandomFloat( -0.25, 0.25 );
		y = RandomFloat( -0.25, 0.25 ) + RandomFloat( -0.25, 0.25 );
		/*
		float r = random->RandomFloat();
		float theta = random->RandomFloat( 0, DEG2RAD(360) );
		SinCos( theta, &x, &y );
