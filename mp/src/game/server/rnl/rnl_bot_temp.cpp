...
class CRnLBot : public CRnLPlayer
{
public:
	bool			m_bBackwards;
...
LINK_ENTITY_TO_CLASS( rnl_bot, CRnLBot );
...
public:
	static CBasePlayer* ClientPutInServerOverride_Bot( edict_t *pEdict, const char *playername )
	{
		...
		CRnLBot *pPlayer = static_cast<CRnLBot *>( CreateEntityByName( "rnl_bot" ) );
		if ( pPlayer )
		{
...
CBasePlayer *BotPutInServer( bool bFrozen )
{
	...
	// This trick lets us create a CRnLBot for this client instead of the CRnLPlayer
	...
	CRnLBot *pPlayer = ((CRnLBot*)CBaseEntity::Instance( pEdict ));
	...
	pPlayer->ChangeTeam( random->RandomInt( TEAM_ALLIES, TEAM_AXIS ) );
	CRnLGameTeam* pTeam = (CRnLGameTeam*)GetGlobalRnLTeam( pPlayer->GetTeamNumber() );
	bool joinedSquad = false;
	if( pTeam )
	{
		...
		if( iSquad != -1 )
		{
			...
			if( pSquad )
			{
				...
				if( iSlot > -1 )
				{
					joinedSquad = pPlayer->ChangeSquad( iSquad, iSlot );
				}
			}
		}
	}
...
void Bot_RunAll( void )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		{
			CRnLBot *pBot = dynamic_cast< CRnLBot* >( pPlayer );
			if ( pBot )
				Bot_Think( pBot );
		}
	}
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CRnLPlayer *fakeclient, CUserCmd &cmd, float frametime )
{
	if ( !fakeclient )
		return;
...
void Bot_UpdateStrafing( CRnLBot *pBot, CUserCmd &cmd )
{
...
void Bot_UpdateDirection( CRnLBot *pBot )
{
...
void Bot_FlipOut( CRnLBot *pBot, CUserCmd &cmd )
{
...
void Bot_HandleSendCmd( CRnLBot *pBot )
{
...
void Bot_ForceFireWeapon( CRnLBot *pBot, CUserCmd &cmd )
{
...
void Bot_SetForwardMovement( CRnLBot *pBot, CUserCmd &cmd )
{
...
void Bot_HandleRespawn( CRnLBot *pBot, CUserCmd &cmd )
{
